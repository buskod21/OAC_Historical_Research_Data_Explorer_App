  ...
  fluidRow(
    tags$div(
      ...
      style = "display: flex; justify-content: center; align-items: center; 
             flex-wrap: wrap; gap: 20px; position: relative; width: 100%;",
      
      tags$div(
        id = "valueBoxes",
        class = "row justify-content-center",
        style = "gap: 20px; padding: 20px; border-radius: 10px; 
      width: 100%; height: auto; min-height: 400px;",
        
        # Adjusted Bootstrap grid for responsive layout
      ...
      # First Image (Explore Network) without background overlay
      tags$div(id = "exploreNetwork", style = "display: none; position: relative; width: 100%; height: 400px;", 
               # Image with 100% width, centered
               tags$div(style = "display: flex; justify-content: center;
                        align-items: center; position: relative; width: 100%; height: 100%;",
                        tags$img(src = "view_network.jpg", width = "100%", height = "100%", 
                                 style = "object-fit: cover;padding: 20px; border-radius: 30px"),
                        
                        # Text with background only for the text block
                        tags$div(style = "position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); 
                               text-align: center; background: rgba(0, 0, 0, 0.5); padding: 20px; border-radius: 10px; color: white",
                                 tags$h1("Explore Networks within the App", 
                                         style = "font-size: 40px; font-weight: bold; text-transform: title;"),  # Increased visibility
                                 tags$p("Discover relationships and connections between authors and keywords", 
                                        style = "font-size: 15px; font-weight: bold; font-color: white;"),
                                 actionButton("go_to_network", 
                                              "Go to Network tab", 
      ...
      # Second Image (Explore Data) without background overlay
      tags$div(id = "exploreData", style = "display: none; position: relative; width: 100%; height: 400px;", 
               # Image with 100% width, centered
               tags$div(style = "display: flex; justify-content: center; align-items: center; position: relative; width: 100%; height: 100%;",
                        tags$img(src = "Explore_data.jpg", width = "100%", height = "100%", 
                                 style = "object-fit: cover; padding: 20px; border-radius: 30px"),
                        
                        # Text with background only for the text block
                        tags$div(style = "position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); 
                               text-align: center; background: rgba(0, 0, 0, 0.5); padding: 20px; border-radius: 10px; color: white",
                                 tags$h1("Explore and Visualize Data", 
                                         style = "font-size: 40px; font-weight: bold; text-transform: title;"),  # Increased visibility
                                 tags$p("Discover various metadata and datasets for diverse studies within OAC", 
                                        style = "font-size: 15px; font-weight: bold;"),
                                 actionButton("go_to_borealis", 
                                              "Go to Data review tab", 
                                              class = "btn btn-lg btn-primary")
                        )
               )
      ),
      ...
      # Dots for manual navigation (pagination dots)
      tags$div(
        id = "carouselNav",
        style = "position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); display: flex; gap: 10px;",
        tags$button(class = "carousel-dot", onclick = "showSection(0);", 
                    style = "width: 15px; height: 15px; border-radius: 50%; background-color: blue; border: none;"),
        tags$button(class = "carousel-dot", onclick = "showSection(1);", 
                    style = "width: 15px; height: 15px; border-radius: 50%; background-color: blue; border: none;"),
        tags$button(class = "carousel-dot", onclick = "showSection(2);", 
                    style = "width: 15px; height: 15px; border-radius: 50%; background-color: blue; border: none;")
      ),
      
      # Adding JavaScript for the carousel functionality
      tags$script(HTML('
      let currentIndex = 0;
      const sections = ["valueBoxes", "exploreNetwork", "exploreData"];
      
      // Show the specific section based on index
      function showSection(index) {
          ...
          dots.forEach((dot, i) => {
              dot.style.backgroundColor = i === index ? "#3c8dbc" : "lightblue";
          });

          currentIndex = index;
      }

      // Auto slide between images every 5 seconds
      function autoSlide() {
      ...
      showSection(0);  // Start with the first section visible
    '))
    )
  ),
