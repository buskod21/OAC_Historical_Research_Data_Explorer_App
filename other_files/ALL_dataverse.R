...
server <- function(input, output, session) {
   ...
   observe({
    req(App_data$OAC_info, input$select_dataverse)  # Ensure these are loaded

    selectedIDs <- input$select_dataverse

    # Content for all IDs in OAC repo is fetched via API call and saved into App_data
    App_data$dataverse_data <- fetch_dataverse_data(selectedIDs)

    # Get detailed metadata for all datasets in OAC repo via API call and saved into App_data
    App_data$detailed_data <- fetch_study_details(App_data$dataverse_data)

    data <- App_data$detailed_data

    # Populate the select  study drop-down based on selected dataverses i.e., detailed_data
    if (!is.null(data) && "Title" %in% names(data)) {
      updateSelectInput(session, "study_select", choices = unique(data$Title))
    }

    # Process the Keywords pulled from detailed_data
    if ("Keywords" %in% names(data)) {  # Check for the correct column name
      App_data$keywords <- data$Keywords %>%
        str_split(";\\s*") %>%  # Split keywords by semicolon and optional spaces
      ...
      character(0) # Return an empty character vector if 'Keywords' column does not exist
    }

    # Process the Authors pulled from detailed_data
    if ("Authors" %in% names(data)) {  # Check for the correct column name
      App_data$authors <- data$Authors %>%
        str_split(";\\s*") %>%  # Split authors by semicolon and optional spaces
        unlist() %>%  # Flatten the list to a vector
        unique() %>%  # Retrieve unique elements
        na.omit() %>%  # Remove NA values
        sort()  # Sort alphabetically
  ...
  # Expression to prepare nodes and edge data based on selected event type
   ...
   # Expression to create edges data based on selected event type
    ...
    # Assign the complete edges data frame to App_data$edges
    App_data$edges <- edges
  })

  # Render the network plot
  output$networkPlot <- renderVisNetwork({
    req(App_data$nodes, App_data$edges)
    visNetwork(App_data$nodes, App_data$edges, width = "100%", height = "800px") %>%
  ...
  }) %>% bindCache(App_data$nodes, App_data$edges,App_data$detailed_data)

  # When an event is selected in the network, update the study select input
  observeEvent(input$selectedEvent, {
    req(input$event_type, App_data$detailed_data)
    ...
    if (!is.null(input$selectedEvent) && !is.na(input$selectedEvent)) {
      ...
      if (input$event_type == "Keywords") {
        event <- App_data$keywords[input$selectedEvent]
        event_column <- "Keywords"
      } else {  # Assume Authors
        event <- App_data$authors[input$selectedEvent]
        event_column <- "Authors"
      }

      # Apply filtering based on the selected node
      filtered_data <- data %>%
        dplyr::filter(stringr::str_detect(.data[[event_column]], stringr::regex(paste0("\\b", event, "\\b"), ignore_case = TRUE)))
  ...
  output$study_details <- DT::renderDataTable({
    req(input$study_select, App_data$detailed_data)  # Ensure that a selection has been made and detailed_data is available
  ...
  observeEvent(input$study_select, {
    ...
    req(input$study_select, App_data$detailed_data)

    # Fetch DOI, file lists, and update inputs
    selected_title <- input$study_select
    selected_doi <- App_data$detailed_data %>%
  ...
  #   req(input$metadata_select, App_data$detailed_data)  # Ensure a file is selected
  #
